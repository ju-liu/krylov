...
class _RecyclingSolver(object):
    ...
    def __init__(self, DeflatedSolver, vector_factory=None):
        ...
        :param DeflatedSolver: a deflated solver from
          :py:mod:`~krylov.deflation`.
        :param vector_factory: (optional) An instance of a subclass of
          :py:class:`krylov.recycling.factories._DeflationVectorFactory`
           ...
           * ``'RitzApproxKrylov'``: uses the approximate Krylov subspace bound
             evaluator :py:class:`krylov.recycling.evaluators.RitzApproxKrylov`.
           * ``'RitzAprioriCg'``: uses the CG :math:`\\kappa`-bound
             (:py:class:`krylov.utils.BoundCG`) as an a priori bound with
             :py:class:`krylov.recycling.evaluators.RitzApriori`.
           * ``'RitzAprioriMinres'``: uses the MINRES bound
             (:py:class:`krylov.utils.BoundMinres`) as an a priori bound with
             :py:class:`krylov.recycling.evaluators.RitzApriori`.
...
class RecyclingCg(_RecyclingSolver):
    ...
    See :py:class:`~krylov.recycling.linsys._RecyclingSolver` for the
